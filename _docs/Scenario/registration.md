...
экране при запуске приложения. Процесс регистрации заключается в заполнении регистрационной формы, которая требует минимально необходимые данные для идентифицирования пользователя при повторном входе в систему. Данная форма включает в себя такие тектовые поля как: 
<br>
---ФИО;<br>
---E-mail;<br>
---Создания пароля;<br>
...
<br>
В момент ввода кодового слова в текстовое поле могут возникнуть те же проблемы с валидность указанные ранее, если проблем не возникает, то аккаут успешно создается и пользователя переадресовывает на экранную форму личного профиля.
<br>
...
---ФИО<br>
---Почта<br>
В случае отказа последует возвращение на экран заполнения формы. Следующий шаг - проверка на дубликат аккаунта, если почта была использована, то пользователь будет проинформирован о уже существующем аккаунте. В случае успешной проверки происходит переадресация на форму создания пароля и его подтверждения, при вводе текста паралельно выполняется проверка на валидность, если:
---Введен неверный формат символов;<br>
---Нарушено ограничение макимального количества символов;<br>
---Поле осталось не заполненным<br>
...
Так же ниже продемонстрированная общая форма регистрации и авторизации пользователя.<br>
...
<p><a href="/images/registration-scenario/log-in.jpg" download>Схема 3. Положтельный сценарий входа в систему</a>
